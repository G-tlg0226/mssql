'use strict'
const debug = require( 'debug' )( 'DBP' ),
	chalk = require( 'chalk' ),
	EventEmitter = require( 'events' ).EventEmitter,
	isPromise = ( obj ) => ( {} ).toString.call( obj ) === '[object Promise]'
...
class Pool extends EventEmitter {
	...
	getClient( request = {} ) {
		...
		if ( isPromise( open ) ) {
			open.then( ( client ) => {
				...
				log( '连接创建成功， ID :', request.uuid )
			} ).catch( ( err ) => {
				log( '连接创建失败，正在尝试重新创建，错误信息 ', err )
	...
	acquire() {
		...
		log( 'stroes.length', this.stores.length )
		for ( let request of this.stores ) {
			console.log( request )
